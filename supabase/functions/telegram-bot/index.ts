...
async function pairRow(userId:string){const {data,error}=await supabase.from('athenaeum_telegram_pairing').select('*').eq('user_id',userId).eq('profile_key','main').maybeSingle();if(error)throw error;return data}
...
Deno.serve(async req=>{
  ...
  try{
    ...
    if(body?.mode==='test'){await sendTest(user.id);return new Response(JSON.stringify({ok:true}),{headers:cors()})}
    if(body?.mode==='status'){const row=await pairRow(user.id),username=await botUsername();return new Response(JSON.stringify({ok:true,connected:!!(row?.enabled&&row?.telegram_chat_id),bot_username:username}),{headers:cors()})}
